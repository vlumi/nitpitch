...
The scaffold is complete and fully green, and the `Tuner` → `Nitpitch` rename is
done:

- 31 tests passing (`make test`, ~0.07 s)
- SwiftLint and swift-format both clean **when run from the repo root**
- Both app targets build; the built app is `Nitpitch.app` with bundle id
  `fi.misaki.nitpitch`, matching the App Store Connect record
- `make run-mac` verified launching a real app with real microphone capture
  (before the rename)
- Clean tree, **no git remote configured yet**
...
2. **Take it to a real violin.** `make run-mac` is the fastest loop. This is the
   ...
   Any of these may send you into `Packages/NitpitchCore/Sources/NitpitchCore/DSP/`
   to retune constants. They're all in `Detection.swift`, deliberately.

## Things that will bite you

- **SourceKit reports `No such module 'NitpitchCore'`** for unindexed files. These
...
- **Run SwiftLint from the repo root.** Its `excluded:` paths are relative to
  the invocation directory, so running it inside `Packages/NitpitchCore` lints the
...
- **Don't create App Store Connect records.** The user does that — and has: the
  `Nitpitch` record exists under bundle id `fi.misaki.nitpitch`. Don't change
  that string; an ASC bundle id can't be edited once the record exists.
