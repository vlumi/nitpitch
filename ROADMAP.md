...
- **Sympathetic-ring rejection for DIAL LIGHTING — attempted 2026-08-22, PARKED with the measurements.** Bowing a violin's G and D sets the open A and E ringing and lights their dials. Not a regression (identical probe numbers at every build from b2 to b16), and the fix is harder than the entry used to claim. Two candidate discriminators were built and measured, both dead ends, both recorded in `RingRejectionTests` so the next attempt starts from the numbers: (1) **ratio** — "suppress what a lower string's partial explains", the `SubharmonicFilter` analogy — is founded on a false premise, since a violin's strings are a FIFTH apart and no neighbour is an integer harmonic of another (G→A is 2.245×); the ring is the A string genuinely vibrating, not a partial in a band. (2) **energy** — ring ratios (0.035, 0.137) INTERLEAVE with deliberate light double stops (0.073, 0.145), so no threshold separates them; at one instant the two are physically identical. What remains is **onset timing** (a ring has no attack of its own), which needs per-string envelope tracking and would misfire on a slow bow INTO a double stop — standard technique, and the case the app exists for. Field severity on b16 is low (iPhone: D+G legible, only E flashing), so the cost/risk is not yet earned. `testAGenuineDoubleStopStillLightsBothDials` is the constraint any attempt must keep.
