...
and uploads in the agreed store order (grid first, the dark twin last).
...
def check_sizes(files, platform):
    ...
    if bad:
        ...
        sys.exit(f"Wrong-sized {platform} capture(s) — expected {want}:\n{lines}\n"
                 "Recapture via `make shots` (it pins the window/simulator size).")
...
def upload(asc, set_id, path):
    ...
    # attribute (as it does for some asset kinds).
