...
#
# NOTE: the simulator has no usable microphone input — it reports a device but
# delivers silence or host audio, so pitch detection cannot be evaluated here.
# Use the sim for layout/navigation only; test detection on a real device.
set -euo pipefail
cd "$(dirname "$0")/.."
